...
const Cart = () => {
  ...
  const [FinalTaxvalue, SetFinalTaxValue] = useState();
  ...
  useEffect(() => {
    ...
    SetFinalTaxValue(Number(((subTotal / 100) * taxValue).toFixed(2)));
  }, [subTotal, taxValue]);
  ...
  const handlerSubmit = async (value) => {
    ...
    try {
      const newObject = {
        ...
        totalAmount: Number(
          (
            Number(subTotal) + Number(((subTotal / 100) * taxValue).toFixed(2))
          ).toFixed(2)
        ),
    ...
    <Layout>
      ...
      >
        <Form layout="vertical" onFinish={handlerSubmit}>
          ...
          <Form.Item name="paymentTax" label="Payment Tax">
            <Select
              defaultValue={`${taxValue}%`}
              onChange={(e) => setTaxValue(e)}
            >
          ...
          <div className="total">
            ...
            <br />
            <span>Tax: ${((subTotal / 100) * taxValue).toFixed(2)}</span>
            <h3>
              Total: $
              {(
                Number(subTotal) +
                Number(((subTotal / 100) * taxValue).toFixed(2))
              ).toFixed(2)}
            </h3>
          </div>
